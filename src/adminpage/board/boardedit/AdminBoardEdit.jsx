...
import { removeBoardFileOnce } from "../../../api/board/removeBoardFileOnce";
import { uploadBoardFile } from "../../../api/board/uploadBoardFile";
import { removeBoardImgOnce } from "../../../api/board/removeBoardImgOnce";
...
export default function AdminBoardEdit() {
  ...
  useEffect(() => {
    ...
    urls?.map((name) =>
      setPreviewFile((prev) => [
        ...prev,
        {
          url: `${process.env.REACT_APP_URL_BOARD}/files/${ID}/${name}`,
          name: name,
          uuid: uuidv4(),
  ...
  useEffect(() => {
    ...
    urls?.map((name) =>
      setPreviewImg((prev) => [
        ...prev,
        {
          url: `${process.env.REACT_APP_URL_BOARD}/images/${ID}/${name}`,
          name: name,
          uuid: uuidv4(),
          lastModified: Math.ceil(Math.random() * 10000),
        },
  ...
  const removeFile = (e) => {
    ...
    updateBoard(board).then(() =>
      removeBoardFileOnce({ id: ID, fileName: fileName })
    );
  };
  ...
  const removeImg = (e) => {
    ...
    updateBoard(board).then(() =>
      removeBoardImgOnce({ id: ID, fileName: fileName })
    );
  };
  const goBack = () => {
    navigate(-1);
